...
class QualityCheckWizard(models.TransientModel):
    _name = 'setu.quality.check.wizard'
    _description = "Wizard for Quality Check Pop Up"

    check_ids = fields.Many2many('setu.quality.check', required=True)
    current_check_id = fields.Many2one('setu.quality.check', required=True)
    ...
    def show_failure_message(self):
        ...
        return {
            ...
            'res_model': 'setu.quality.check.wizard',
            'views': [(self.env.ref('setu_quality_control.setu_quality_check_wizard_form_failure').id, 'form')],
            'target': 'new',
            'res_id': self.id,
            'context': {
                **self.env.context,
