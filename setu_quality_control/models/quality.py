...
class QualityPoint(models.Model):
    ...
    def check_execute_now(self):
        ...
        if self.measure_frequency_type == 'all':
            ...
            checks = self.env['setu.quality.check'].search([
                ('point_id', '=', self.id),
    ...
    def action_see_quality_checks(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("setu_quality_control.setu_quality_check_action_main")
    ...
    def action_see_spc_control(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("setu_quality_control.setu_quality_check_action_spc")
...
class QualityCheck(models.Model):
    ...
    def action_see_alerts(self):
        ...
        if len(self.alert_ids) == 1:
            ...
            action = self.env["ir.actions.actions"]._for_xml_id("setu_quality_control.setu_quality_alert_action_check")
    ...
    def action_open_quality_check_wizard(self, current_check_id=None):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("setu_quality_control.action_quality_check_wizard")
...
class QualityAlert(models.Model):
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        custom_values['name'] = self.env['ir.sequence'].next_by_code('setu.quality.alert') or _('New')
...
class ProductTemplate(models.Model):
    ...
    quality_control_point_qty = fields.Integer(compute='_compute_quality_check_qty', groups='setu_quality.group_setu_quality_user')
    quality_pass_qty = fields.Integer(compute='_compute_quality_check_qty', groups='setu_quality.group_setu_quality_user')
    quality_fail_qty = fields.Integer(compute='_compute_quality_check_qty', groups='setu_quality.group_setu_quality_user')
    ...
    def action_see_quality_control_points(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("setu_quality_control.setu_quality_point_action")
...
class ProductProduct(models.Model):
    ...
    def _count_quality_checks(self):
        ...
        quality_checks_by_state = self.env['setu.quality.check']._read_group(domain, ['quality_state'], ['__count'])
    ...
    def _count_quality_points(self):
        ...
        query = self.env['setu.quality.point']._where_calc([('company_id', '=', self.env.company.id)])
        self.env['setu.quality.point']._apply_ir_rules(query, 'read')
        ...
        points = self.env['setu.quality.point'].search(['|',('product_ids', 'in', self._ids), ('product_category_ids','in', parent_category_ids)])
        return len(points)
