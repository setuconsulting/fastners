...
class StockMoveLine(models.Model):
    ...
    check_ids = fields.One2many('setu.quality.check', 'move_line_id', 'Checks')
    ...
    def _create_check(self):
        check_values_list = []
        quality_points_domain = self.env['setu.quality.point']._get_domain(
        ...
        for ml in self:
            ...
            quality_points = self.env['setu.quality.point'].sudo().search([('id', 'in', list(quality_points_product | quality_points_all_products))])
            for quality_point in quality_points:
        ...
        if check_values_list:
            self.env['setu.quality.check'].sudo().create(check_values_list)
