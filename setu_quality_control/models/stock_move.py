...
class StockMove(models.Model):
    ...
    def _create_quality_checks(self):
        ...
        for picking, moves in pick_moves.items():
            ...
            quality_points_domain = self.env['setu.quality.point']._get_domain(moves.product_id, picking.picking_type_id, measure_on='product')
            quality_points = self.env['setu.quality.point'].sudo().search(quality_points_domain)
        ...
        self.env['setu.quality.check'].sudo().create(check_vals_list)

    def _create_operation_quality_checks(self, pick_moves):
        check_vals_list = []
        for picking, moves in pick_moves.items():
            quality_points_domain = self.env['setu.quality.point']._get_domain(moves.product_id, picking.picking_type_id, measure_on='operation')
            quality_points = self.env['setu.quality.point'].sudo().search(quality_points_domain)
    ...
    def _action_cancel(self):
        ...
        to_unlink = self.env['setu.quality.check'].sudo()
