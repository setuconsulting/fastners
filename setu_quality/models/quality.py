...
class TestType(models.Model):
    _name = "setu.quality.point.test_type"
...
class QualityPoint(models.Model):
    ...
    def _get_default_team_id(self):
        ...
        return self.team_id._get_quality_team(self.env['setu.quality.alert.team']._check_company_domain(company_id))

    def _get_default_test_type_id(self):
        domain = self._get_type_default_domain()
        return self.env['setu.quality.point.test_type'].search(domain, limit=1).id
    ...
    check_ids = fields.One2many('setu.quality.check', 'point_id')
    test_type_id = fields.Many2one('setu.quality.point.test_type', 'Test Type', help="Defines the type of the quality control point.",
    ...
    def _compute_check_count(self):
        check_data = self.env['setu.quality.check']._read_group([('point_id', 'in', self.ids)], ['point_id'], ['__count'])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'name' not in vals or vals['name'] == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('setu.quality.point') or _('New')
...
class QualityAlertTeam(models.Model):
    ...
    def _compute_check_count(self):
        check_data = self.env['setu.quality.check']._read_group([('team_id', 'in', self.ids), ('quality_state', '=', 'none')], ['team_id'], ['__count'])
        check_result = {team.id: count for team, count in check_data}
        for team in self:
            team.check_count = check_result.get(team.id, 0)

    def _compute_alert_count(self):
        alert_data = self.env['setu.quality.alert']._read_group([('team_id', 'in', self.ids), ('stage_id.done', '=', False)], ['team_id'], ['__count'])
    ...
    def _get_quality_team(self, domain):
        team_id = self.env['setu.quality.alert.team'].search(domain, limit=1).id
        if team_id:
            return team_id
        else:
    ...
    def _alias_get_creation_values(self):
        ...
        values['alias_model_id'] = self.env['ir.model']._get('setu.quality.alert').id
        if self.id:
...
class QualityAlertStage(models.Model):
    _name = "setu.quality.alert.stage"
    ...
    team_ids = fields.Many2many('setu.quality.alert.team', string='Teams')


class QualityCheck(models.Model):
    _name = "setu.quality.check"
    ...
    point_id = fields.Many2one(
        'setu.quality.point', 'Control Point', check_company=True)
    ...
    team_id = fields.Many2one(
        'setu.quality.alert.team', 'Team', required=True, check_company=True)
    company_id = fields.Many2one(
        'res.company', 'Company', required=True, index=True,
        default=lambda self: self.env.company)
    alert_ids = fields.One2many('setu.quality.alert', 'check_id', string='Alerts')
    alert_count = fields.Integer('# Quality Alerts', compute="_compute_alert_count")
    note = fields.Html('Note')
    test_type_id = fields.Many2one(
        'setu.quality.point.test_type', 'Test Type',
        required=True)
    ...
    def _compute_alert_count(self):
        alert_data = self.env['setu.quality.alert']._read_group([('check_id', 'in', self.ids)], ['check_id'], ['__count'])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'name' not in vals or vals['name'] == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('setu.quality.check') or _('New')
            if 'point_id' in vals and not vals.get('test_type_id'):
                vals['test_type_id'] = self.env['setu.quality.point'].browse(vals['point_id']).test_type_id.id
            if 'point_id' in vals and not vals.get('note'):
                vals['note'] = self.env['setu.quality.point'].browse(vals['point_id']).note
        return super().create(vals_list)
...
class QualityAlert(models.Model):
    ...
    def _get_default_stage_id(self):
        ...
        if not team_id and self.env.context.get('active_model') == 'setu.quality.alert.team' and\
                self.env.context.get('active_id'):
            team_id = self.env['setu.quality.alert.team'].browse(self.env.context.get('active_id')).exists().id
        domain = [('team_ids', '=', False)]
        if team_id:
            domain = OR([domain, [('team_ids', 'in', team_id)]])
        return self.env['setu.quality.alert.stage'].search(domain, limit=1).id
    ...
    stage_id = fields.Many2one(
        'setu.quality.alert.stage', 'Stage', ondelete='restrict',
    ...
    company_id = fields.Many2one(
        'res.company', 'Company', required=True, index=True,
        default=lambda self: self.env.company)
    reason_id = fields.Many2one('setu.quality.reason', 'Root Cause')
    tag_ids = fields.Many2many('setu.quality.tag', string="Tags")
    ...
    check_id = fields.Many2one('setu.quality.check', 'Check', check_company=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'name' not in vals or vals['name'] == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('setu.quality.alert') or _('New')
        return super().create(vals_list)
